...
let calSelectedDate = currentDate;
...
async function setState(updater, reRender = true) {
    updater(appState); await saveState(); if (reRender) renderActiveTab();
}
...
function initData() {
    ...
    appState.finance.forEach(f => {
        ...
        f.expenses.forEach(e => {
            ...
            if (!e.payment) e.payment = "現金";
            if (!e.tags) e.tags =[];
            if (!e.rating) e.rating = 2;
            if (e.isAdvance === undefined) e.isAdvance = false;
        });
    });
...
function ensureDate(dateStr) {
    if (!appState.dates[dateStr]) appState.dates[dateStr] = {};
    const d = appState.dates[dateStr];
    if (!d.tasks) d.tasks =[];
...
function processAutoSubscriptions() {
    ...
    ensureFinanceMonth(monthStr);
    const finMonth = appState.finance.find(f => f.month === monthStr);
    let updated = false;
...
function setupEventListeners() {
    document.querySelectorAll('.nav-item').forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            activeTab = e.currentTarget.dataset.target;
...
function renderToday() {
    ...
    mealKeys.forEach((k, i) => {
        ...
        const sel = el('select', { onChange: e => setState(s => {
            const catId = e.target.value;
            s.dates[currentDate].meals[k].categoryId = catId;
            const catInfo = s.settings.mealCategories.find(x => x.id === catId);
            s.dates[currentDate].meals[k].amount = catInfo ? catInfo.price : 0;
        }) },
        ...
        const row = el('div', {className: 'meal-grid'});
        row.append(el('div', {className: 'meal-icon'}, mealLabels[i]), sel, inpAmt, inpMemo);
        mc.appendChild(row);
    });
}
...
function renderCalendar() {
    ...
    for (let d = 1; d <= lastDate; d++) {
        ...
        let hasTask = data.tasks && data.tasks.length > 0;
        let hasMemo = data.todayMemo || data.reflection?.keep || data.reflection?.problem;
        let hasExp = exps.length > 0;
        ...
        const dayEl = el('div', { className: classes, onClick: () => { calSelectedDate = dateStr; renderCalendar(); } }, el('div', {}, d), dots);
        if(isNMD) dayEl.appendChild(el('div', {className:'nmd-icon'}, "👑"));
        body.appendChild(dayEl);
    }
    
    document.getElementById('cal-preview-container').style.display = 'block';
    document.getElementById('preview-title').innerText = calSelectedDate;
    const content = document.getElementById('preview-content'); content.innerHTML = "";
    const pData = appState.dates[calSelectedDate] || {};
    const pFin = appState.finance.find(f => f.month === calSelectedDate.slice(0,7));
    const pExps = pFin ? pFin.expenses.filter(e => e.date === calSelectedDate.slice(8,10)) :[];
    
    if (pData.tasks && pData.tasks.length > 0) {
        const div = el('div', {className:'preview-box'}); div.appendChild(el('h3', {}, "✅ タスク"));
        pData.tasks.forEach(t => div.appendChild(el('div', {style:'padding:4px 0'}, `${t.done?'[済]':'[未]'} ${t.text}`)));
        content.appendChild(div);
    }
    if (pData.meals) {
        const div = el('div', {className:'preview-box', style:'border-left:3px solid var(--primary);'}); 
        div.appendChild(el('h3', {}, "🍚 食事"));
        const getName = (cId) => { const c = appState.settings.mealCategories.find(x=>x.id===cId); return c ? c.name : '未記録'; };
        div.appendChild(el('div', {}, `朝: ${getName(pData.meals.bf?.categoryId)} / 昼: ${getName(pData.meals.lu?.categoryId)} / 夜: ${getName(pData.meals.di?.categoryId)}`));
        content.appendChild(div);
    }
    if (pExps.length > 0) {
        const div = el('div', {className:'preview-box', style:'border: 1px solid var(--warn); background:rgba(255,69,58,0.05);'});
        div.appendChild(el('h3', {}, "¥ 支出"));
        pExps.forEach(e => div.appendChild(el('div', {className:'flex', style:'justify-content:space-between; padding:4px 0'}, 
            el('span', {}, e.name), el('span', {style:'color:var(--warn); font-family:monospace;'}, `¥${Number(e.amount).toLocaleString()}`)
        )));
        content.appendChild(div);
    }
}
...
function renderFinance() {
    ...
    sel.innerHTML = "";
    appState.finance.sort((a,b)=>a.month>b.month?-1:1).forEach(f => sel.appendChild(el('option', { value: f.month }, f.month)));
...
function drawFinanceMonth(monthStr) {
    ...
    let mealMonthTotal = 0;
    const[yy, mm] = monthStr.split('-');
    const daysInMonth = new Date(yy, mm, 0).getDate();
    for(let i=1; i<=daysInMonth; i++) {
        let dStr = `${yy}-${mm}-${String(i).padStart(2,'0')}`;
        let data = appState.dates[dStr];
        if(data && data.meals) mealMonthTotal += getN(data.meals.bf?.amount) + getN(data.meals.lu?.amount) + getN(data.meals.di?.amount);
    }

    const totalIncome = getN(f.income) + getN(f.extraIncome);
    const deduct = getN(f.fixed) + getN(f.loan) + getN(f.savings) + getN(f.investment) + mealMonthTotal;
    const budget = totalIncome - deduct;
    
    let expTotal = 0, creditTotal = 0;
    (f.expenses||[]).forEach(e => {
        if (!e.isAdvance) expTotal += getN(e.amount);
        if (e.payment === "クレカ") creditTotal += getN(e.amount);
    });
    ...
    const sumCard = el('div', {className:'card finance-summary'},
        ...
        el('div', {className:'grid-2', style:'margin-top:15px; text-align:left;'},
            el('div', {}, el('div', {style:'font-size:0.8rem; color:var(--gray);'}, "変動費 残高"), el('div', {style:`font-weight:bold; font-size:1.2rem; color:${balance<0?'var(--warn)':'var(--sub)'}`}, `¥${balance.toLocaleString()}`)),
            el('div', {}, el('div', {style:'font-size:0.8rem; color:var(--gray);'}, "今日使える額"), el('div', {style:`font-weight:bold; font-size:1.2rem; color:${dailyLimit<0?'var(--warn)':'#fff'}`}, `¥${dailyLimit.toLocaleString()}`))
        ),
    ...
    leftCol.appendChild(el('div', {className:'card'},
        ...
        el('div', {style:'margin-top:15px;'}, ...(f.expenses||[]).sort((a,b)=>b.date-a.date).map(e => 
            el('div', {className:'expense-item', onClick:()=>openExpenseModal(e, mIdx)},
                el('div', {}, 
                    el('div', {className:'flex'}, el('span', {style:'color:var(--gray); font-size:0.8rem; width:20px;'}, e.date), el('strong', {}, e.name), e.isAdvance ? el('span', {className:'badge sub'}, "立替") : ""),
                    el('div', {style:'font-size:0.75rem; color:var(--gray); margin-top:4px; display:flex; gap:6px;'}, el('span', {className:'badge'}, e.category), el('span', {className:'badge'}, e.payment))
                ),
                el('div', {style:'text-align:right;'},
                    el('div', {style:'font-family:monospace; color:var(--warn); font-weight:bold;'}, `¥${Number(e.amount).toLocaleString()}`),
    ...
    const createAcc = (title, contentEls) => {
        ...
        const cont = el('div', {className:'acc-content', style:'padding:16px;'}, ...contentEls);
        wrap.append(head, cont); return wrap;
    };

    rightCol.appendChild(createAcc("収入・固定費 設定",[
        el('div', {className:'grid-2'},
            el('div', {}, el('label',{},"月収(基本)"), el('input', {type:'number', value:f.income, onChange:e=>setState(s=>s.finance[mIdx].income=e.target.value)})),
            el('div', {}, el('label',{},"臨時収入"), el('input', {type:'number', value:f.extraIncome, onChange:e=>setState(s=>s.finance[mIdx].extraIncome=e.target.value)})),
            el('div', {}, el('label',{},"固定費(家賃等)"), el('input', {type:'number', value:f.fixed, onChange:e=>setState(s=>s.finance[mIdx].fixed=e.target.value)})),
            el('div', {}, el('label',{},"ローン等"), el('input', {type:'number', value:f.loan, onChange:e=>setState(s=>s.finance[mIdx].loan=e.target.value)}))
        )
    ]));

    rightCol.appendChild(createAcc("📱 通信GB アラート管理", [
        ...(f.carriers||[]).map((c, cIdx) => {
            const isAlert = (getN(c.used) / (getN(c.limit)||1)) >= 0.8;
            return el('div', {className:`grid-3 ${isAlert?'gb-alert':''}`, style:'align-items:end; margin-bottom:10px; padding:8px; border-radius:var(--radius); background:#1c1c1e;'},
...
function openExpenseModal(exp, mIdx) {
    ...
    if (exp) {
        ...
        document.getElementById('exp-name').value = exp.name; document.getElementById('exp-amount').value = exp.amount;
        document.getElementById('exp-category').value = exp.category; document.getElementById('exp-payment').value = exp.payment;
        document.getElementById('exp-tags').value = exp.tags.join(','); document.getElementById('exp-advance').checked = exp.isAdvance;
        document.getElementById('exp-memo').value = exp.memo; document.getElementById('exp-rating').value = exp.rating;
    } else {
        document.getElementById('exp-id').value = ""; document.getElementById('exp-date').value = currentDate.slice(8,10);
        document.getElementById('exp-name').value = ""; document.getElementById('exp-amount').value = "";
        document.getElementById('exp-tags').value = ""; document.getElementById('exp-memo').value = "";
        document.getElementById('exp-advance').checked = false; document.getElementById('exp-rating').value = 2;
    }
    document.querySelectorAll('.rating-star').forEach(el => el.classList.toggle('active', el.dataset.val == document.getElementById('exp-rating').value));
...
function saveExpenseFromModal() {
    ...
    const expObj = {
        id: id ? parseFloat(id) : Date.now(), date: String(document.getElementById('exp-date').value).padStart(2,'0'),
        name: document.getElementById('exp-name').value, amount: Number(document.getElementById('exp-amount').value),
        category: document.getElementById('exp-category').value, payment: document.getElementById('exp-payment').value,
        tags: document.getElementById('exp-tags').value.split(',').map(t=>t.trim()).filter(t=>t), rating: Number(document.getElementById('exp-rating').value), isAdvance: document.getElementById('exp-advance').checked, memo: document.getElementById('exp-memo').value
    };
    setState(s => { const arr = s.finance[currentMIdx].expenses; if(id) { const idx = arr.findIndex(x=>x.id==id); if(idx>=0) arr[idx] = expObj; } else arr.push(expObj); });
    document.getElementById('modal-expense').classList.remove('active');
}

function openAnnualModal() {
    const modal = document.getElementById('modal-annual'); modal.classList.add('active');
    const years =[...new Set(appState.finance.map(f => f.month.slice(0,4)))].sort().reverse();
    const sel = document.getElementById('annual-year-select'); sel.innerHTML = "";
    years.forEach(y => sel.appendChild(new Option(y+"年", y)));
    const drawChart = () => {
        const targetYear = sel.value; const months =["01","02","03","04","05","06","07","08","09","10","11","12"];
        let incomeData =[], expData =[], totalSave = 0;
        months.forEach(m => {
            const f = appState.finance.find(x => x.month === `${targetYear}-${m}`);
            if(f) { const inc = Number(f.income) + Number(f.extraIncome); const exps = f.expenses.filter(e=>!e.isAdvance).reduce((sum, e) => sum + Number(e.amount), 0) + Number(f.fixed) + Number(f.loan); const sav = inc - exps; incomeData.push(inc); expData.push(exps); totalSave += sav; } else { incomeData.push(0); expData.push(0); }
        });
        ...
        charts.annual = new Chart(document.getElementById('annual-chart'), { type: 'bar', data: { labels: months.map(m=>m+"月"), datasets:[ { label: '支出', data: expData, backgroundColor: '#ff453a' }, { label: '収入', data: incomeData, backgroundColor: '#0a84ff' } ]}, options: { responsive: true, scales: { x:{stacked:false}, y:{beginAtZero:true} } } });
    };
...
function renderSettings() {
    ...
    document.getElementById('btn-bulk-meals').onclick = () => {
        ...
        setState(s => {
            let curr = new Date(sDate); const end = new Date(eDate);
            while(curr <= end) {
                const ds = curr.toISOString().split('T')[0];
                if (!s.dates[ds]) s.dates[ds] = { tasks:[], reflection:{keep:"",problem:"",try:""}, todayMemo:"", meals:{ bf:{categoryId:'none',amount:0,memo:''}, lu:{categoryId:'none',amount:0,memo:''}, di:{categoryId:'none',amount:0,memo:''} } };
                if (!s.dates[ds].meals) s.dates[ds].meals = { bf:{categoryId:'none',amount:0,memo:''}, lu:{categoryId:'none',amount:0,memo:''}, di:{categoryId:'none',amount:0,memo:''} };
                s.dates[ds].meals.bf = { categoryId: bfId, amount: getP(bfId), memo: "" };
                s.dates[ds].meals.lu = { categoryId: luId, amount: getP(luId), memo: "" };
                s.dates[ds].meals.di = { categoryId: diId, amount: getP(diId), memo: "" };
    ...
    appState.settings.subscriptions.forEach((sub, idx) => {
        subDiv.appendChild(el('div', {className: 'flex list-item'}, el('span', {className:'drag-handle'}, "☰"),
            ...
            el('button', {className: 'btn-del', onClick: () => setState(s => s.settings.subscriptions.splice(idx, 1))}, "✕")
        ));
    });
    ...
    document.getElementById('btn-add-payment').onclick = () => setState(s => s.settings.paymentMethods.push("新規支払い"));
}
...
async function importData(e) {
    ...
    try {
        ...
        if(json.dates) Object.keys(json.dates).forEach(k => { if(!appState.dates[k]) appState.dates[k] = json.dates[k]; else Object.assign(appState.dates[k], json.dates[k]); });
        if(json.memos) appState.memos = json.memos;
        if(json.finance) json.finance.forEach(f => { let exist = appState.finance.find(x => x.month === f.month); if(exist) Object.assign(exist, f); else appState.finance.push(f); });
        initData(); await saveState(); alert("復元・マージ完了しました"); location.reload();
